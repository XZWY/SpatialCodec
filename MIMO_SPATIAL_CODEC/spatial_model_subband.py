...
class SpatialDecoder(torch.nn.Module):
    def __init__(self, channels=[128, 128, 128, 128, 128, 256, 256]):
        ...
        self.conv_list.apply(init_weights)
        self.conv_post = weight_norm(nn.Conv2d(128, 8*2, (5,5), (1,1), padding=(2,2)))
        self.conv_post.apply(init_weights)
    ...
    def forward(self, x):
        ...
        x = self.conv_post(x) # bs, 14, n_frames, n_freqs
        
        bs, _, n_frames, n_freqs = x.shape
        x = x.reshape(bs, 8, 2, n_frames, n_freqs).permute(0,1,4,3,2).contiguous() # bs, 7, n_freqs, n_frames, 2
        x = torch.view_as_complex(x).reshape(bs*8, n_freqs, n_frames)
        x = torch.istft(x, n_fft=640, hop_length=320, win_length=640, window=self.window.to(x.device), center=True, normalized=False, onesided=None, length=None, return_complex=False)
        x = x.reshape(bs, 8, -1)
        return x
